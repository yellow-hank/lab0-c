...
void format_size(char *buf, struct stat *stat, int len)
{
    if (S_ISDIR(stat->st_mode)) {
        ...
        if (size < 1024) {
            ...
            snprintf(buf, len, "%.1fK", (double) size / 1024);
        } else if (size < 1024 * 1024 * 1024) {
            snprintf(buf, len, "%.1fM", (double) size / 1024 / 1024);
        } else {
            snprintf(buf, len, "%.1fG", (double) size / 1024 / 1024 / 1024);
        }
    }
}
...
void handle_directory_request(int out_fd, int dir_fd, char *filename)
{
    ...
    while ((dp = readdir(dir)) != NULL) {
        ...
        format_size(size, &statbuf, sizeof(size));
...
void client_error(int fd, int status, char *msg, char *longmsg)
{
    ...
    snprintf(buf + strlen(buf), sizeof(buf), "Content-length: %zu\r\n\r\n",
             strlen(longmsg));
    snprintf(buf + strlen(buf), sizeof(buf), "%s", longmsg);
...
void serve_static(int out_fd, int in_fd, http_request *req, size_t total_size)
{
    ...
    snprintf(buf + strlen(buf), sizeof(buf), "Cache-Control: no-cache\r\n");
    // snprintf(buf + strlen(buf), sizeof(buf), "Cache-Control: public,
    // max-age=315360000\r\nExpires: Thu, 31 Dec 2037 23:55:55 GMT\r\n");

    snprintf(buf + strlen(buf), sizeof(buf), "Content-length: %lu\r\n",
             req->end - req->offset);
    snprintf(buf + strlen(buf), sizeof(buf), "Content-type: %s\r\n\r\n",
             get_mime_type(req->filename));
...
int open_web(int argc, char **argv)
{
